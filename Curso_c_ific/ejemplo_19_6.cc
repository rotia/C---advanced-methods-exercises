...
int main() {
  // Task specification
  auto                b = std::async(ask_the_brits);
  auto                f = std::async(ask_the_french);
  auto                g = std::async(ask_the_germans);
  auto                i = std::async(ask_the_italians);
  std::future<string> s = std::async(ask_the_spaniards);
