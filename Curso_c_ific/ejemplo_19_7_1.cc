...
int main() {
  // Task specification
  auto b = std::async(std::launch::deferred, ask_the_brits);
  auto f = std::async(std::launch::deferred, ask_the_french);
  auto g = std::async(std::launch::deferred, ask_the_germans);
  auto i = std::async(std::launch::deferred, ask_the_italians);
  auto s = std::async(std::launch::deferred, ask_the_spaniards);
